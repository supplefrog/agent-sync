...
When taste is unresolved for a new or redesigned surface, offer a quick visual comparison before substantial implementation: a few meaningfully different treatments of the same real content, with source links to explore further. Use [references/taste-selection.md](references/taste-selection.md). Let the user select, combine, reject or request nearby alternatives without needing design jargon. If the user delegates the choice or wants no comparison, choose one provisional direction. Do not interrupt routine repairs or an already approved direction with another style selection.
...
2. **Set the direction.** For a new or redesigned surface, use the user's references and feedback; consult `references/design-judgment.md` when useful. Resolve the few decisions that distinguish the direction, without a compulsory thesis schema. When taste is uncertain, use the lightweight selection path above; delegated choice needs no comparison board. Translate the subject into useful relationships, not a literal costume.
