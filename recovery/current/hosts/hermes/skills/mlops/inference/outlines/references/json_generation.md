# Legacy pre-1.0 JSON Generation Guide

Version-bound compatibility material: the generator functions below are not
current Outlines APIs. Use [current recipes](../SKILL.md) for a new project.
In current Outlines, wrap raw JSON schemas with `outlines.types.JsonSchema`,
regex strings with `outlines.types.Regex`, and grammar strings with
`outlines.types.CFG`; parse returned strings explicitly. Do not assume a
backend enforces every Pydantic constraint or that valid JSON is factual.
