...
- **Constrain generation** rather than relying only on post-generation checks
...
Use the current recipes on this page. The bundled reference manuals below
contain pre-1.0 compatibility material, not current executable recipes.
Check the selected backend's supported output types and token limit; truncated
output, transport failure and unsupported schema constraints still need handling.
Schema validity does not establish factual correctness. Do not install models
or incur inference charges merely to load or update this skill.
...
**Benefits:**
- **Backend-dependent cost**: benchmark compilation and token filtering
- **Speed improvement**: Fast-forward through deterministic paths
- **Constrained structure**: validate completed output and handle truncation
...
#### Regex (wrap the pattern in `Regex`)

```python
# Generate text matching a regex pattern
from outlines.types import Regex
phone = model("Generate phone number:", Regex(r"[0-9]{3}-[0-9]{3}-[0-9]{4}"))
...
age = int(model("Person's age:", int))
price = float(model("Product price:", float))
```
...
# vLLM >= 0.12 server; use the actual configured endpoint/model.
client = OpenAI(base_url=os.environ["VLLM_BASE_URL"], api_key=os.environ["VLLM_API_KEY"])
model = outlines.from_vllm(client, os.environ["VLLM_MODEL"])
...
# Transformers batching has an explicit batch method.
prompts = [f"Extract from: {t}" for t in texts]
outputs = model.batch(prompts, Person, max_new_tokens=100)
...
Configure tensor parallelism, GPU placement and quantization on the vLLM
server, then wrap its OpenAI-compatible client as above. `from_vllm` is not
the in-process `LLM` constructor route. For offline inference, consult the
current `VLLMOffline` documentation before choosing that distinct API.
Servers older than 0.12 can silently ignore structured-output arguments.
...
| Runtime overhead | Backend-dependent; measure | Backend-dependent | Backend-dependent | Backend-dependent |
...
- Need generation-time structural constraints
...
**Speed:** measure the selected backend/schema/model combination rather than
assuming zero overhead or a fixed speedup. Compilation and caching matter.
...
**Accuracy:** validate complete responses, handle truncation and backend
errors, and verify factual content separately. Bound any retries; do not
silently spend on repeated generation.
...
- `references/json_generation.md` - Legacy pre-1.0 JSON patterns, only for deliberately pinned old projects
- `references/backends.md` - Legacy pre-1.0 backend configuration
- `references/examples.md` - Legacy pre-1.0 examples, not current recipes
...
- https://dottxt-ai.github.io/outlines/latest/features/models/vllm_offline/
