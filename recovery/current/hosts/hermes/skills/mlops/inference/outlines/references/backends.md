# Legacy pre-1.0 Backend Configuration

Compatibility reference only for deliberately pinned pre-1.0 projects.
Do not execute these deprecated `outlines.models` / `outlines.generate`
recipes with current Outlines. Use [current recipes](../SKILL.md) and the
linked official backend documentation instead; retain device, quantization,
throughput and deployment concepts only where they still apply.
