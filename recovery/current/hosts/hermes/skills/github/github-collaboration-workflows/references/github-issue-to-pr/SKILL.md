...
An existing PR counts as a viable integration surface only after a velocity-relative staleness check. Inspect its draft/review/check state, last substantive author or maintainer activity, merge-base, and ahead/behind counts against current default branch; distinguish recent comments from renewed implementation work. Compare calendar age with repository movement over the same interval—a PR that is weeks old in a slow repository may be current, while one hundreds of commits behind in a fast repository is stale. If it is active and reasonably current, collaborate there. If it is dormant or materially diverged enough that maintainers are unlikely to notice or revive it, open a focused current-main PR, explicitly credit/link the earlier work, and explain why a fresh integration surface is warranted rather than merely leaving a comment. Do not apply a universal day or commit cutoff.

Done when you know every open PR and recent commit touching this issue and have classified each candidate as active/current or stale/diverged using observable evidence.
