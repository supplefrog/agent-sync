...
Install dependency into the interpreter used for this script:
    uv pip install --python "<interpreter path>" youtube-transcript-api
"""
...
def fetch_transcript(video_id: str, languages: list = None):
    ...
    try:
        ...
        print(f'Error: youtube-transcript-api not installed. Run: uv pip install --python "{sys.executable}" youtube-transcript-api',
              file=sys.stderr)
        sys.exit(1)
