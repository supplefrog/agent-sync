---
name: powerpoint
description: Create, read, edit and style .pptx decks with Python or PptxGenJS.
version: 1.1.0
...
### Advanced authoring (local extension)

For precise rich text, character spacing, shadows, SVG and styled merged
tables, load [advanced/AUTHORING.md](advanced/AUTHORING.md). Use its pinned
PptxGenJS package and executable probe, not an unpinned global installation.
The Python helpers remain the default for inspection, editing and templates.

Activation does not require installing a renderer now. When real presentation
work begins, select a suitable viewer/renderer and arrange its dependencies;
LibreOffice is one option, not the default choice. Render and visually inspect
every slide before calling the deck finished, correcting overflow, overlap,
font substitution and weak composition. Structural checks cannot establish
visual quality or improvement over another authoring workflow. Without a
renderer, treat output as a draft, not a visually verified final deck. Do not
upload private decks to a rendering service without authorization.

### Python and optional rendering tools
