...
- After extraction: check the JSON is non-empty and spot-check a known string or cell. For parser-sensitive documents such as resumes, compare PyMuPDF `page.get_text(sort=True)` with pypdf `page.extract_text()` on the same PDF; retain raw outputs and inspect heading/word spacing and accent artifacts. Report disagreement as parser-specific, not proof of employer ATS rejection.
- When checking font embedding with pypdf, inspect `/FontDescriptor` on both the resource font and its `/DescendantFonts`; Type0 composite fonts commonly store embedded font streams on descendants. Check `/FontFile`, `/FontFile2`, or `/FontFile3` separately from `/ToUnicode` so Unicode mapping is not mistaken for font embedding.
- For compiled LaTeX, retain compiler warnings alongside page renders. An overfull box can exceed its intended text width without leaving the physical page; page-boundary checks do not establish overlap-free layout.
