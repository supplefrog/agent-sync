...
Set the terminal working directory to `<repo-root>/ui-tui`, where `<repo-root>` comes from the active install/source discovery in `debugging-hermes-tui-commands`, then run:
...
Launch the TUI with the Node inspector enabled from process start; this works without POSIX `pgrep`/`SIGUSR1` assumptions:

```bash
NODE_OPTIONS='--inspect=127.0.0.1:9229' hermes --tui
```

Read the inspector URL from the startup output or `http://127.0.0.1:9229/json/list`, then attach from another terminal:

```bash
node inspect ws://127.0.0.1:9229/<uuid>
```

If port 9229 is occupied, choose another loopback port. Interacting with the TUI continues execution until a breakpoint pauses it.

### Debugging `_SlashWorker` / PTY child processes

Those are Python, not Node—switch to this skill’s `references/python-debugpy.md` branch. Only Node portions (Ink UI, TUI gateway client, and tests under `ui-tui/`) use this branch.

## Running Vitest Tests Under the Debugger

Set the terminal working directory to `<repo-root>/ui-tui`, then run a single test file paused on entry:
