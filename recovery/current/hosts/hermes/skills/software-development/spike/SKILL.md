...
metadata:
  hermes:
    ...
    related_skills: [sketch, delegation-workflows, plan]
---
...
For agent/tool benchmark spikes, use the isolation and scoring checklist in `references/agent-benchmark-spikes.md`: create pristine failing fixture repos, run each agent only on disposable copies with isolated homes/caches, and independently verify results with tests/diffs instead of trusting agent self-reports. Treat weak-model failures as harness/model sanity data before drawing class-level conclusions.
