...
> **Version-gated legacy detail.** Start with the current [official webhook documentation](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/webhooks) and installed `hermes webhook --help`. The commands, flags, routes, defaults, delivery behaviors, systemd/log recipes, and `~/.hermes` paths below were retained from the baseline for implementation history and have not been revalidated as a set. Resolve the active `HERMES_HOME`/profile and gateway host first. Use Linux/systemd commands only when the gateway actually runs there; use only details confirmed for the installed version. Never weaken signature checks, authorization, rate limits, or network exposure controls for convenience.
