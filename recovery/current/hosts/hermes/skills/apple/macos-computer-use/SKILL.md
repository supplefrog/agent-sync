...
- **"blocked pattern in type text"** — You tried to `type` a shell command
  ...
  `sudo rm -rf`, etc.). Respect the block. Do not split, encode, or switch tools to evade it. Use a permitted alternative that removes the blocked behavior, or report the restriction.
