...
npx hyperframes preview                     # long-lived browser preview (port 3002)
...
`preview` is long-lived and can retain Chrome workers. Start it through the
host's tracked background-process tool and retain its handle. When review
ends, stop that owned process tree through the same tool and verify both the
server and its captured child workers exited. Closing a terminal pane alone
does not stop its process.

If a native fallback is necessary, verify the current command, start time
and ancestry before terminating the specific owned PID/tree; a stale PID may
have been reused. On Windows, `taskkill.exe /PID <verified-PID> /T` targets a
tree. On Unix, target a process group only when it was explicitly created and
recorded by this task; do not assume `setsid` exists on macOS. Never use global
`pkill`, `killall`, or kill every `chrome-headless-shell` process. See
[references/troubleshooting.md](references/troubleshooting.md#runaway-cpu-from-a-leftover-preview).

## Pitfalls

- **Leaving `preview` running** — stop its recorded process tree after review; do not use global process-name cleanup.
