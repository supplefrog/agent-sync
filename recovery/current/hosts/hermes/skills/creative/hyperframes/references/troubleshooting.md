...
## Runaway CPU from a leftover preview

**Symptom:** Chrome render workers remain busy after preview review is finished.

**Cause:** `npx hyperframes preview` is a long-lived server. GPU-less hosts may use software WebGL, and its child workers can remain expensive while the server is alive.

**Fix:** use the tracked background-process handle to stop the preview and
its owned workers, then verify their exit. If the handle is unavailable,
inspect command/start-time/ancestry before targeting a specific owned process
tree. Do not trust an old PID without checking for reuse, assume `setsid` is
installed on macOS, or use `pkill`, `killall`, or process-name-wide cleanup.

**Avoid:** keep previews in a dedicated terminal, record ownership, stop them after review, and use one-shot `render` for final output.
