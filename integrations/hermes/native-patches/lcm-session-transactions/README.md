...
Prerequisites: restore the native LCM checkout at the exact base revision in `manifest.json`, its dependencies and Hermes plugin registration. That installation is outside Agent Signal's declarative restore. The recorded remote identifies the source repository; availability of every historical commit on a future remote is not guaranteed.
...
Agent Signal records exact file readbacks for the two production modules. If the native prerequisite is absent or differs, recovery reports failure; it does not claim a whole agent was reconstructed. Tests, personal databases, credentials and existing sessions are not executed or read by this hash check.
